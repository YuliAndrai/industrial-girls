...
Ejemplos:
  ...
  ./scripts/task-init.sh feature shared fix-ui-elements --mode parent --owner czambrano --issue IGW-38
  ./scripts/task-init.sh bugfix shared login-redirect-fix --mode parent --owner czambrano --issue IGW-171
...
if [[ "${ASK_MODE}" == "ask" ]]; then
  ...
  if [[ "${BRANCH_MODE}" =~ ^(parent|spec)$ ]]; then
    prompt_required "What Linear issue key anchors the work (for example IGW-149)?" "" ISSUE_KEY
