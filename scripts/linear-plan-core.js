...
function normalizeIssueId(rawIssueId) {
  ...
  if (!value) {
    throw new Error("`--issue` is required (example: --issue IGW-149).");
  }
  ...
  throw new Error("`--issue` must look like IGW-149 or 149.");
}
...
function usage() {
  return [
    ...
    "",
    "Usage:",
    "  npm run linear:plan -- --issue IGW-149 --type feature --owner czambrano --slug my-feature --title \"My feature\" --goal \"...\" --spec \"S01|Planning SPEC|Scope|Validation\"",
    "",
    "Options:",
    "  --issue <IGW-149>        Parent Linear issue identifier",
