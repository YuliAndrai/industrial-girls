...
Ejemplos:
  ...
  ./scripts/git-start.sh feature shared fix-ui-elements --mode parent --owner czambrano --issue IGW-38
  ./scripts/git-start.sh bugfix shared login-redirect-fix --mode parent --owner czambrano --issue IGW-171
  ./scripts/git-start.sh epic shared admin-collections-console --mode parent --owner czambrano --issue EPIC-011
  ./scripts/git-start.sh SPEC hero-copy-tightening --mode spec --owner czambrano --issue IGW-38 --base feature/czambrano-IGW-38-fix-ui-elements
...
normalize_issue_key() {
  ...
  echo "❌ Issue inválido: ${raw}. Usa formato IGW-149."
  exit 1
}
