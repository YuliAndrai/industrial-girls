...
const tools = [
  {
    ...
    description: "Fetch a Linear issue by identifier, for example IGW-168.",
    inputSchema: {
      type: "object",
      properties: {
        id: { type: "string", description: "Linear issue identifier, for example IGW-168." }
      },
      required: ["id"]
    }
  },
      ...
      case "initialize":
        send({
          ...
          result: {
            protocolVersion,
            capabilities: { tools: {} },
            serverInfo: { name: "industrial-girls-linear-mcp", version: "1.0.0" }
          }
        });
        return;
