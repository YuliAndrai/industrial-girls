...
description: Linear MCP Bridge - migrated from knowledge/
tags: [guides]
timestamp: 2026-07-20T04:23:56Z
resource: https://github.com/jeisonsosablockdev/industrial-girls/blob/develop/knowledge/guides/linear-mcp-bridge.md
---

# Linear MCP Bridge

This repository now includes a local MCP bridge for Linear so Codex and other MCP-capable tools can talk to Linear using the same Industrial Girls workflow rules that already govern branch naming, slice planning, and issue-state transitions.
...
- `README.md` documents the integration next to the other MCP servers already used in Industrial Girls.

## Notes

- If the API key is missing, the server will fail fast instead of pretending it is connected.
- The bridge is intentionally scoped to the Industrial Girls workflow needs first, rather than trying to mirror every Linear feature at once.
